...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option.setName("título").setDescription("Escreva seu título!").setRequired(true),
        )
        .addStringOption((option) =>
            option.setName("descrição").setDescription("Descreva sua notícia.").setRequired(true),
        )
        .addStringOption((option) =>
            option.setName("imagem").setDescription("Url de uma imagem.").setRequired(true),
        )
        .addStringOption((option) =>
        ...
        .addStringOption((option) =>
            option.setName("subdescrição").setDescription("Adicione uma descrição ao subtitulo."),
        )
        .addStringOption((option) =>
    ...
    async execute(interaction) {
        ...
        try {
            ...
            if (interaction.member.roles.cache.get(channelIds["index"])) {
                action.client.channels.cache
                    .find((channel) => channel.id == channelIds["index"])
